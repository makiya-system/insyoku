...
*/
  function zen_break_string($string, $len, $break_char = '-') {
    if (strlen($string) != mb_strlen($string, CHARSET)) return $string;
    $l = 0;
    $output = '';
...
// $raw_date needs to be in this format: YYYY-MM-DD HH:MM:SS
  function zen_date_long($raw_date) {
    ...
    $format = (stristr(PHP_OS, 'win')) ? mb_convert_encoding(DATE_FORMAT_LONG, 'CP932', CHARSET) : DATE_FORMAT_LONG;
    $retVal = strftime($format, mktime($hour, $minute, $second, $month, $day, $year));
    if (stristr(PHP_OS, 'win')) return mb_convert_encoding($retVal, CHARSET, 'CP932');
...
function zen_trunc_string($str = "", $len = 150, $more = 'true') {
  ...
  if ($str != "") {
    // check to see if there are any spaces left
    if (!mb_substr_count($str , " ", CHARSET)) {
      if ($more == 'true') $str .= "...";
      return $str;
    }
    // backtrack
    /*
    while(mb_strlen($str, CHARSET) && ($str[mb_strlen($str, CHARSET)-1] != " ")) {
      $str = mb_substr($str, 0, -1, CHARSET);
    }
    */
    $str = mb_substr($str, 0, -1, CHARSET);
    if ($more == 'true') $str .= "...";
