...
//echo 'GV chk_gv_amount: ' . $chk_gv_amount . ' $order_total: ' . $order_total . '<br>';
  ...
  function install() {
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('ソート順', 'MODULE_ORDER_TOTAL_GV_SORT_ORDER', '840', '表示の整列順を設定します。<br />数値が小さいほど上に表示されます。', '6', '2', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('ギフト券の承認', 'MODULE_ORDER_TOTAL_GV_QUEUE', 'false', 'ギフト券購入後にショップの承認を必要とする。<br />購入されたギフト券は「承認待ちギフト券」のリストに追加され、承認を受けるまで利用できません。', '6', '3','zen_cfg_select_option(array(\'true\', \'false\'), ', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('「承認待ち ギフト券」ボタンを管理画面のヘッダに表示', 'MODULE_ORDER_TOTAL_GV_SHOW_QUEUE_IN_ADMIN', 'true', '「承認待ち ギフト券」ボタンを管理画面の全てのページのヘッダに表示しますか？<br>（対象がない場合には true でもボタンは表示されません。また、 false でも「注文管理」画面には表示されます。）', '6', '3','zen_cfg_select_option(array(\'true\', \'false\'), ', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function ,date_added) values ('送料を含める', 'MODULE_ORDER_TOTAL_GV_INC_SHIPPING', 'true', '送料をギフト券での支払対象に含めるかどうかを設定します。', '6', '5', 'zen_cfg_select_option(array(\'true\', \'false\'), ', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function ,date_added) values ('税金を含める', 'MODULE_ORDER_TOTAL_GV_INC_TAX', 'true', '税金をギフト券での支払対象に含めるかどうかを設定します。', '6', '6','zen_cfg_select_option(array(\'true\', \'false\'), ', now())");
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('税種別', 'MODULE_ORDER_TOTAL_GV_TAX_CLASS', '0', 'ギフト券に適用される税種別を設定します。<br />上記の「税金を再計算する」で「Credit Note」を選択した場合に設定してください。', '6', '0', 'zen_get_tax_class_title', 'zen_cfg_pull_down_tax_classes(', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function ,date_added) values ('ギフト券への税金分の利用', 'MODULE_ORDER_TOTAL_GV_CREDIT_TAX', 'false', 'ギフト券購入時に支払われた税金分も支払に利用可能としますか?（「ギフト券」という商品に税金を加算している場合）', '6', '8','zen_cfg_select_option(array(\'true\', \'false\'), ', now())");
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('ギフト券の特価販売', 'MODULE_ORDER_TOTAL_GV_SPECIAL', 'false', 'ギフト券を特価で販売することを許可しますか？', '6', '3','zen_cfg_select_option(array(\'true\', \'false\'), ', now())");
  }
  /**
   * Enter description here...
   *
