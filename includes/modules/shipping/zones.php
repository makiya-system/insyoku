...
// shipping adjustment
    ...
    function install() {
      global $db;
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('地域料金', 'MODULE_SHIPPING_ZONES_STATUS', 'False', '地域料金による配送を有効にしますか？', '6', '0', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('計算方式', 'MODULE_SHIPPING_ZONES_METHOD', 'Weight', '送料計算のベースを、Weight(重量)、Price（価格)、Item（商品点数)から選択してください。', '6', '0', 'zen_cfg_select_option(array(\'Weight\', \'Price\', \'Item\'), ', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('税種別', 'MODULE_SHIPPING_ZONES_TAX_CLASS', '0', '配送料に適用される税種別を選んでください。', '6', '0', 'zen_get_tax_class_title', 'zen_cfg_pull_down_tax_classes(', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('税率の計算ベース', 'MODULE_SHIPPING_ZONES_TAX_BASIS', 'Shipping', '配送料にかかる税金の計算ベースを選択します。<br /><br />Shipping - 顧客の送付先住所に基づく<br />Billing - 顧客の請求先住所に基づく<br />Store - ショップの所在住所に基づく(送付先/請求先がショップ所在地と同じ地域の場合に有効)', '6', '0', 'zen_cfg_select_option(array(\'Shipping\', \'Billing\', \'Store\'), ', now())");
      ...
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('対象外となる国の指定：　2文字のISO 国名コードをカンマ区切りのリストにしてください。', 'MODULE_SHIPPING_ZONES_SKIPPED', '', '以下の国は対象外:', '6', '0', 'zen_cfg_textarea(', now())");
