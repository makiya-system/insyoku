...
// final check for display of Free Options
    ...
    function install() {
      global $db;
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('配送料無料オプションを有効にする', 'MODULE_SHIPPING_FREEOPTIONS_STATUS', 'True', 'このモジュールは他の配送モジュールと並んで配送料無料オプションを表示するために使用します。<br />常に表示、注文合計金額、注文重量または注文点数を基準に設定できます。この無料オプションモジュールは配送料無料が表示されているときは非表示になります。<br><br>対象合計金額を0円以上に設定し、なおかつ0円以下にも設定すると配送料無料以外の全てのモジュールに有効になります。<br><br>注意: 合計、重量、点数の全ての設定を空欄にするとこのモジュールは無効になります。<br><br>注意: 配送料無料オプションは重量０で無料に設定されていると表示されません。配送料無料モジュールを参照してください<br><br>
