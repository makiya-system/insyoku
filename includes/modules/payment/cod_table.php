<?php
/**
 * @copyright Copyright 2006-2019 Bigmouse, Inc
 * @version $Id: cod_table.php 1 2019-04-18 13:53 bigmouse-nfox $
 **/
...
// disable the module if the order only contains virtual products
      ...
      if (MODULE_PAYMENT_COD_TABLE_SHIPPING_MODULE != '') {
        $this->enabled = false;
        $shipping_modules = explode(',', MODULE_PAYMENT_COD_TABLE_SHIPPING_MODULE);
    ...
    function calc_fee($total_cost) {
      $table_fee = preg_split("/[:,]/" , MODULE_PAYMENT_COD_TABLE_COST);
      $f_find = false;
      $this->n_fee = 0;
...
// class methods
    ...
    function install() {
      ...
      values ('代金引換払いを有効にする', 'MODULE_PAYMENT_COD_TABLE_STATUS', 'False', 
      '代金引換による支払いを有効にする<br />支払い総額に代金引換手数料を加算したい時には<br />注文合計モジュールの ot_cod_table を有効にしてください。', '6', '1', 'zen_cfg_select_option(array(\'True\', \'False\'), ',now());");
      ...
      values ('代引手数料の計算表', 'MODULE_PAYMENT_COD_TABLE_COST', 
      '9999:330,29999:440,99999:660,299999:1100', '支払い金額に連動する代引手数料. 例: 9999:330,29999:440,99999:660,299999:1100 ... 9999円まで330円, 29999円まで440円, ...', '6', '2', now())");

      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) 
      values ('表示の整列順', 'MODULE_PAYMENT_COD_TABLE_SORT_ORDER', 
      '0', '表示の整列順を設定できます。数字が小さいほど上位に表示されます.', '6', '0' , now())");
      ...
      values ('適用地域', 'MODULE_PAYMENT_COD_TABLE_ZONE', 
      '0', '適用地域を選択すると、選択した地域のみで利用可能となります.', '6', '3', 'zen_get_zone_class_title', 'zen_cfg_pull_down_zone_classes(', now())");
      ...
      values ('初期注文ステータス', 'MODULE_PAYMENT_COD_TABLE_ORDER_STATUS_ID', 
      '0', '設定したステータスが受注時に適用されます.', '6', '4', 'zen_cfg_pull_down_order_statuses(', 'zen_get_order_status_name', now())");

      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) 
      values ('配送方法選択', 'MODULE_PAYMENT_COD_TABLE_SHIPPING_MODULE', 
      ...
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) 
      values ('配送先と支払い先', 'MODULE_PAYMENT_COD_TABLE_SHIPPING_PAYMENT', 
      'False', '配送先と支払い先が違っていても代金引換モジュールを有効にしますか？<br />True - 有効にする。<br />False - 無効にする。', '6', '1', 'zen_cfg_select_option(array(\'True\', \'False\'), ',now());");
    }
