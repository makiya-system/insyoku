  ...
  if ($error == false) {
    ...
    if ($_POST['action'] == 'update') {
      ...
      if ( (isset($_POST['primary']) && ($_POST['primary'] == 'on')) || ($_GET['edit'] == $_SESSION['customer_default_address_id']) ) {
        ...
        $sql_data_array[] = array('fieldName'=>'customers_telephone', 'value'=>$entry_telephone, 'type'=>'stringIgnoreNull');
        if (ACCOUNT_FAX_NUMBER == 'true') {
          $sql_data_array[] = array('fieldName'=>'customers_fax', 'value'=>$entry_fax, 'type'=>'stringIgnoreNull');
        }
