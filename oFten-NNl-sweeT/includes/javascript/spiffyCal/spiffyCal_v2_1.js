...
function ctlSpiffyCalendarBox(strVarName, strFormName, strTextBoxName, strBtnName, strDefaultValue, intBtnMode) {
    ...
    function makeCalendar(intWhatMonth, intWhatYear, bViewOnly) {
        ...
        if (scNN6) {
            strOutput += strTemp + '<table class="cal-Table table"><tr>';
        } else {
            strOutput += '<table class="cal-Table table">' + strTemp + '<tr>';
        }

        if (!(bViewOnly)) {
            strOutput += '<td class="cal-HeadCell text-center" colspan="7">';
            strOutput += '<a href="javascript:' + this.varName + '.clearDay();"><i class="fa fa-arrow-left fa-lg"></i></a>&nbsp;&nbsp;<a href="javascript:' + this.varName + '.scrollMonth(-1);" class="cal-DayLink">&lt;</a>&nbsp;<SELECT class="cal-ComboBox" NAME="cboMonth" onChange="' + this.varName + '.changeMonth();">';
        ...
        for (intLoop = 0; intLoop < 7; intLoop++) {
            strOutput += '<td class="cal-HeadCell text-center" valign="middle">' + msDOW[intLoop] + '<\/td>';
        }
    ...
    function writeControl() {
        ...
        strHold = '<div class="input-group">';
        strHold += '<input class="cal-TextBox form-control" type="text" name="' + this.textBoxName + '"' + strTemp + 'size="12" value="' + this.getSelectedDate() + '">';
        if (!scIE) {
        ...
        if ((this.focusClick == false) || (this.focusClick == true && this.hideButton == false)) {
            strHold += '<div class="input-group-addon">';
...
// put these next 2 lines in when the tiny cal btns seem to randomly disappear
    ...
    function clearDay() {
        ...
        eval('document.' + this.formName + '.' + this.textBoxName).className = "cal-TextBox form-control";
        if (this.required) {
            eval('document.' + this.formName + '.' + this.textBoxName).className = "cal-TextBoxInvalid";
        }
    }
