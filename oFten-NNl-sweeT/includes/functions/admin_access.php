...
function zen_validate_user_login($admin_name, $admin_pass)
{
    ...
    if ($result['reset_token'] != '')
    {
      ...
      if ($expired_token > 0)
      {
        ...
        } else
        {
          ...
          } elseif (SETTING_DEVELOPMENT_ENVIRONMENT != 'true')
          {
            $error = true;
            $expired = true;
  ...
  if ($error == FALSE && PADSS_PWD_EXPIRY_ENFORCED == 1 && $result['pwd_last_change_date'] < date('Y-m-d H:i:s', ADMIN_PASSWORD_EXPIRES_INTERVAL) && SETTING_DEVELOPMENT_ENVIRONMENT != 'true')
  {
    $expired = true;
    $error = true;
  }
